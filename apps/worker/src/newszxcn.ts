...
export async function createSharedInboxLink(request: Request, env: Env): Promise<Response> {
  ...
  const timestamp=nowIso(); await env.DB.prepare("INSERT INTO shared_inbox_links (id,user_id,short_key_hash,short_key_ciphertext,external_grant_id,grant_id,mailbox_id,mailbox_address,folder_ids_json,window_minutes,expires_at,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(newId("sil"),user.id,await sha256(shortKey),await encryptNewSzxcn(env,shortKey),grantExternal,grant.id,body.mailboxId,box.address,JSON.stringify(body.folderIds),body.windowMinutes,body.expiresAt??null,"active",timestamp,timestamp).run();
  return successJson({ link: { shortUrl:`https://dingyue.xzys.me/s/${shortKey}`, grantId: grant.id } }, { status: 201 });
}
